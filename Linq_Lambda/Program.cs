...
namespace Linq_Lambda
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var r3 = products.Where(p => p.Name[0] == 'C' || p.Name[0] == 'c')
                .Select(p => new { p.Name, p.Price, CategoryName = p.Category.Name });
            Print("Name of products that start with C", r3);

            var r4 = products.Where(p => p.Category.Tier == 1)
                .OrderBy(p => p.Price)
                .ThenBy(p => p.Name);
            Print("Tier 1 ordered by price and then by name", r4);

            var r5 = r4.Skip(2).Take(4);
            Print("Tier 1 ordered by price and then by name Skip 2 Take 4", r5);
            ...
            var r14 = products.Where(p => p.Category.Id == 5)
                ...
                .DefaultIfEmpty(0.0)
                .Average();
            Console.WriteLine("Category 5 Avegare Prices: " + r14);

            var r15 = products.Where(p => p.Category.Id == 1)
                .Select(p => p.Price)
                .Aggregate((x, y) => x + y);
            Console.WriteLine("Category 1 Aggregate by sum: " + r15);

            var r16 = products.Where(p => p.Category.Id == 5)
                .Select(p => p.Price)
                .Aggregate(0.0, (x, y) => x + y);
            Console.WriteLine("Category 5 Aggregate by sum: " + r16);
